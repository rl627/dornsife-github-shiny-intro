...
**Simple analysis in this section will examine the relationship between neighborhood level % poverty and whether a neighborhood has a healthy grocery store in the city of Austin, TX** We will first visualize this data and then do some simple models. The data used for the analysis is pulled and previewed here:
...
One of the uses of Shiny is that it enables us to build a web application that allows for accessible exploration high dimension data while using existing R code. **We will now recycle the data and R code used in this analysis to build a Shiny app that displays results across multiple cities, outcomes and exposures.**
